...
整理下来有几处判断和我原来的预期不同：Harness 质量和测试覆盖度对成功率的影响，比换更贵的模型更大；调试 Agent 行为时，优先查工具定义，大多数工具选择错误出在描述上；评测系统的问题往往比 Agent 本身的问题更难发现，评测坏了还一直改 Agent，方向从一开始就偏了。
...
新能力基本通过两种方式接入：扩展工具集，或把状态外化到文件和数据库，系统提示随之调整。循环体不应变成状态机，模型负责推理，外部系统负责状态和边界，这个分工定下来之后，核心循环逻辑就很少需要改动了。
...
大多数 AI 系统拆开看都是这几种模式的组合，很多场景不需要完整的 Agent 自主权，两三种模式搭起来就够了，适合任务的设计才是对的设计。
...
Harness 是围绕 Agent 构建的测试、验证与约束基础设施，至少包含四个部分：验收基线、执行边界、反馈信号和回退手段。

模型重要，但决定系统能不能稳定运行的，往往是这些外围工程条件。这个判断在代码编写这类高可验证任务上最成立，开放式研究、多轮协商这类弱验证任务里，模型上限仍然是瓶颈。

### OpenAI 的 Agent 优先开发实践

3 个工程师 5 个月写了百万行代码，将近 1500 个 PR，约是传统开发速度的 10 倍。这个速度来自几个具体的工程决策，不是模型有多强：
...
图里按任务清晰度和验证自动化程度把任务分成四个象限。右上角目标明确、结果可自动验证，是 Agent 能发挥的区域；左上角任务清楚但验收还得人盯，吞吐量上限是人的审查速度；右下角有自动化反馈但目标模糊，系统会往错误方向高效跑；左下角两样都缺，Agent 基本起不到作用。

Harness 的目标是把任务推进右上角，用机器可执行的判断标准替代人工盯盘。

---

## 3. 上下文工程为什么决定稳定性

Transformer 的注意力复杂度是 $O(n^2)$，上下文越长，关键信号越容易被噪声稀释，实践里最常见的失效模式叫 Context Rot：无关内容占到上下文大头后，Agent 决策质量就会下滑。很多看起来是模型能力不足的问题，追下去都是上下文组织不当。

### 上下文为什么要分层

窗口长度通常不是问题，信息密度才是。偶尔用的东西每次都加载，稳定规则和动态状态混在一起，模型能看到的内容越来越多，真正有用的部分越来越难被注意到。
...
LLM 推理时，Transformer attention 会为每个 token 计算 Key-Value 对，如果当前请求的输入前缀和之前某次请求完全一致，这部分 KV 就不需要重新计算，直接从缓存读取，这就是 Prompt Caching 的底层原理。命中的前提是精确前缀匹配，一个 token 不同就会破坏，所以缓存友好的设计核心是稳定性，系统提示、工具定义、长文档这类在多轮请求里基本不变的内容天然适合缓存，动态信息（当前时间、用户输入、工具调用结果）放在后面，不影响前缀的稳定性。
...
Skills 的思路是：系统提示只保留索引，完整知识按需加载。
...
路由准确率差距不大，但每个启用的 Skill 描述符都常驻上下文，Skill 一多，长描述的累积成本很可观。第二个是精度：描述太短（`help with backend`）等于任何后端工作都能触发，路由会乱。有效的描述符是路由条件，"何时该用我"比"我能做什么"重要。
...
压缩阶段最常见的问题是保留优先级设错了，摘要长度反而不是关键。LLM 会优先删那些看起来还能重新获取的内容，早期 tool output 最先被移除，与之相关的架构决策、约束理由和失败路径也容易一并丢失。最好在 `CLAUDE.md` 或等价文档里明确写出压缩时的保留优先级：
...
工具问题多数出在选不对、描述看不懂、返回值没用，或者出了错 Agent 不知道怎么改，数量不够反而不常见。
...
工具设计大致经历了三个阶段，早期做法是直接把现有 API 封装成工具扔给模型，模型选错时，问题通常出在工具的设计视角：给工程师设计的，不是给 Agent 设计的。
...
工具设计对 Agent 的影响就像 HCI 对人的影响一样直接。评估一个工具，不只是看能不能调用，更要看调用错了之后能不能自己修回来。

差的实现参数模糊、错误不可修正、定义实现分离：
...
调试 Agent 时先查工具定义，大多数选择错误出在描述不准确。工具数量也要克制，能用 Shell 处理的、只需静态知识的、更适合 Skill 的，都不用新增工具。
...
Agent 没有原生的跨会话记忆。会话结束，上下文清空，下次启动从零开始。要让系统保持跨会话的一致性，记忆层必须单独设计，它是基础设施，不是事后可以补上的功能。
...
这个设计可读、可改、可检索。Markdown 文件直接查看修订，搜索时按相关性拉取，不用把整个记忆库塞进上下文。对大多数 Agent 来说，结构化 Markdown 加关键词搜索已经够用，规模超过几千条、真的需要语义相似度时再考虑向量检索。

### 记忆整合如何触发并回退

记忆分层之后，下一步是解决时机问题：什么时候整合，整合失败怎么办。
...
关键在于流程可回退，不在于摘要写得多好。系统只移动指针，不删原始消息，整合失败也能回到存档继续工作。

---

## 6. 如何逐步放开 Agent 自主度

自主度意味着 Agent 能在更长时间跨度内稳定推进任务。要达到这个目标，先要补齐三类基础设施：跨 session 续跑、单 session 内的进度约束，以及慢速 I/O 的后台接入。

### 长任务如何跨 session 继续

长任务最常见的失败发生在 session 边界，不是单步报错。session 结束时任务还没做完，即使有 compaction 也挡不住两类问题：在单个 session 里试图做完整个应用、结果上下文先耗尽；或者只做完一部分，下一轮无法准确恢复现场，过早判断完成。
...
自主度提高后，真正拖慢主循环的通常是外部 I/O：文件操作、网络请求、长耗时命令，这些一旦阻塞，执行节奏就会变差。

把慢速 subprocess 放到后台线程，通过通知队列在下一轮 LLM 调用前注入结果。主循环每轮开始前检查新结果，再决定继续、等待还是调整计划。这比把整个 loop 改成复杂 async runtime 更稳，也更容易维护。

---

## 7. 多 Agent 如何组织

多 Agent 看起来是并行问题，工程上先要解决的是隔离和协作，对应两种工作模式。

指挥者模式是同步协作，人与单个 Agent 紧密互动，每一轮都要调整决策，缺点也很明显，session 一结束，context 就没了，产出物也是短暂的。

统筹者模式是异步委派，人在开始时设定目标，中间让多个 Agent 并行工作，最后再审查产出，这样人只在起点和终点出现，中间产出会变成分支、PR 这类可持久化工件，多 Agent 的主要价值在于把人的持续参与变成对工件的最终审核，而不是单纯多开几个模型。
...
先要有三样东西：协议、任务图、隔离边界。主 Agent 通过 JSONL 队列分派任务，子 Agent 只回传摘要，搜索和调试细节留在自己的上下文里，`.tasks/` 记录任务图，`.worktrees/` 隔离文件修改。顺序不能反：协议先定，隔离先做，再谈协作和并行。
...
评测决定 Agent 做得对不对。很多团队把这步往后放，结果是改了 Prompt 不知道变没变好，换了模型不知道有没有退化，最后只剩一组无法解释的波动数字。评测的核心是测试用例、评分标准和自动验证，难的不是有没有分数，而是分数能不能反映真实质量。
...
这张图里有三组概念需要记住。第一组：`task` 任务、`trial` 单次运行、`grader` 评分器，对应测什么、跑多少次、怎么打分。第二组：`transcript` 完整执行记录和 `outcome` 环境最终结果，两者都要看，只看一边会漏东西。第三组：`agent harness` 被评测的运行框架和 `evaluation harness` 评测基础设施，后者负责跑任务、打分、汇总，`evaluation suite` 是一批任务的集合，是评测的原材料。
...
Pass@k 回答「这个 Agent 理论上能不能做到」，Pass^k 回答「已有功能有没有被改坏」。混用容易误判：回归测试过松漏问题，能力评测过严让每次小改都告警。
...
看到 Agent 表现下降就立刻改 Agent，是常见误区。评测系统先出了问题的话，拿到的是失真信号，改的方向从一开始就偏了，甚至会把本来正常的部分改坏。

评测系统常见出错来源：运行环境资源不足导致进程被杀、评分器本身有 bug 把正确答案判成失败、测试用例和生产场景脱节、只看聚合分数漏掉某一类任务系统性变差。这些表现和模型退化几乎一样，很难从结果数字上区分。
...
全量在线评测成本高，纯随机采样容易错过关键 Trace。更稳妥的做法是对 10%~20% 的 Trace 运行评测，按规则选而不是随机：
...
OpenClaw 把前面这些原则落到了一个可运行系统里：上下文分层、Skills 延迟加载、结构化通信协议和文件系统状态，在这里都能找到对应实现。
...
开放 Shell 权限后，`git push`、`rm`、数据库写入都可能被触发，安全边界必须先于功能。三个问题要先有答案：谁能用、能在哪用、做了什么可以追踪。
...
除了权限、路径和审计，还要补两层兜底：防 Prompt 注入，和防模型服务故障。
