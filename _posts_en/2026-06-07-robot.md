...
![Embodied intelligence across perception, space, action, and torque](/images/2026/robot/00-perception-space-action-torque.png)
...
Breaking it into a data flow made debugging much easier. Many of the problems turned out to involve hardware: false wake-ups, network timeouts, servo angles, unstable power. The hardware-side issues can almost be written as a troubleshooting table.
...
<table>
    <tr>
     <td width="33%">
           <img src="https://gw.alipayobjects.com/zos/k/gu/gOR0gb.png" width="600" alt="Soldering tools, sensors, circuit boards, and wiring prepared for the desktop robot" />
        </td>
        <td width="33%">
           <img src="https://cdn.tw93.fun/blog/pic/26410.jpg" width="600" alt="Internal layout of the desktop robot with controller board, sensors, battery, and wiring" />
        </td>
        <td width="33%">
           <img src="https://cdn.tw93.fun/blog/pic/IMG_050944.JPG" width="600" alt="Assembled desktop robot displaying its listening state" />
        </td>
    </tr>
</table>
...
This is the boundary of my small robot dog. It can turn language into action, but it doesn't know where that motion will take it. It has no pose, no map, and no occlusion handling. It still can't work out how to follow a command like "take two steps left."
...
After 2025, the high and low-level split became clearer. Google DeepMind's Gemini Robotics is one example. ER 1.5 handles understanding and task decomposition. 1.5 turns each step into action. Google also released an On-Device version for local low-latency use, which can adapt to new tasks with 50 to 100 demonstrations.
...
Latency looks different depending on where it happens. If the brain is slow, the robot feels sluggish. If the cerebellum is slow, balance suffers and a small touch may knock it down. If the body layer is slow, motors jitter and heat up.
...
If I had to compress embodied AI into four words, I would use perception, space, action, and torque. The difficulty roughly increases in that order. AI perception is already strong. Spatial ability is still catching up. We're only starting to make progress on action. Torque brings the work back to motors, structure, contact, and power. The closer AI moves to the physical world, the less the model can solve alone. More of the remaining work becomes hardware.
