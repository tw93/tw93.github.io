...
The real issue is usually noise: useful information drowned in irrelevant content.
...
The practical takeaway is straightforward: if you want Claude to stop and ask, give it a dedicated tool.
...
Examples include whether formatting should run, whether protected files can be modified, and whether to notify after task completion.
...
In practice, any single layer has gaps. CLAUDE.md rules alone get ignored, hooks alone can't handle judgment calls.
...
Claude is often reasonably good at writing these rules for itself, and repeated mistakes do usually become less frequent over time. Still, review the file periodically. More on keeping it healthy in section 14.
...
One practical test matters more than most: if you cannot clearly articulate what "done" looks like, the task is probably not ready for autonomous execution by Claude.
