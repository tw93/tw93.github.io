...
summary: I'm sharing what I've learned from six months of using Claude Code across two accounts. It covers context management, Skills, Hooks, Subagents, prompt caching, and CLAUDE.md design, with a focus on making collaboration more reliable and easier to control and check.
categories: Share
lang: en-US
---
...
This article is written for engineers who already use Claude Code and want workflows that are more predictable, more controllable, and easier to verify.
...
Six months of heavy Claude Code use, two accounts, roughly $40 per month. I suppose you could call that tuition.

At first I treated it like a chatbot. Things went sideways pretty fast: context kept getting messy, tools multiplied while outcomes got worse, rule sets grew longer while compliance dropped. I spent some time working through those failures and studying how Claude Code actually operates, and finally understood where things were going wrong.
...
| `Skills` | On-demand methods, tells Claude "how to do it" |
...
This article walks through each layer.

---

## 1. Execution Model

<img src="https://cdn.tw93.fun/blog/upic/QrTwEC.svg" width="1000px" alt="Agent Loop">
...
After using it for a while, I realized I was rarely stuck because the model wasn't smart enough. More often, I'd given it the wrong context, or it had written something I couldn't reliably check or undo.
...
<img src="https://cdn.tw93.fun/blog/upic/1auitO.svg" width="1000px" alt="Context Loading">
...
A typical MCP server such as GitHub can expose 20-30 tool definitions at roughly 200 tokens each, which is **4,000-6,000 tokens**. Connect five servers and you may be spending roughly **25,000 tokens (12.5%)** on fixed overhead alone. I was surprised when I first worked out how much that was. When there's a lot of code to read, that 12.5% really matters.
...
<img src="https://cdn.tw93.fun/blog/upic/DOPgjN.png" width="1000px" alt="/context command output showing token breakdown by source">
...
I came across [RTK (Rust Token Killer)](https://www.rtk-ai.app/) and the idea clicked immediately: filter command output before it reaches Claude, keeping only what's relevant to the next decision. For `cargo test`, instead of thousands of lines:
...
<img src="https://cdn.tw93.fun/blog/upic/2EQvo4.png" width="1000" alt="Plan Mode workflow separating read-only exploration from implementation" />
...
<img src="https://cdn.tw93.fun/blog/upic/oEVuiS.png" width="800" alt="Claude Code Plan Mode exploring a Kaku context-menu feature before implementation" />
...
### Keep Descriptors Short, Every Skill Has a Context Cost
...
- One skill covering review, deploy, debug, docs, and incident response, five different things
...
<img src="https://cdn.tw93.fun/blog/upic/KIWJSq.png" width="1000px" alt="Finding the sweet spot">
...
<img src="https://cdn.tw93.fun/blog/upic/JHqSz5.png" width="1000px" alt="Improving Elicitation & the AskUserQuestion tool">
...
If you want Claude to stop and ask, give it a dedicated tool. Flags and output-format conventions are much easier for the model to skip.

**Todo Tool Evolution**:

<img src="https://cdn.tw93.fun/blog/upic/hXrzJK.png" width="1000px" alt="Updating with Capabilities - Tasks & Todos">
...
<img src="https://cdn.tw93.fun/blog/upic/5J83L4.png" width="1000px" alt="Hooks configuration interface">
...
<img src="https://cdn.tw93.fun/blog/upic/ToAEL8.png" width="1000px" alt="Hooks intervention points in execution flow">

In a 100-edit session, saving 30-60 seconds each time adds up to 1-2 hours. That adds up. **Limit output length** (`| head -30`) to avoid hook output polluting context. For a more systematic approach to output noise across all commands, see the RTK note in section 3.
...
In practice, any single layer has gaps. CLAUDE.md rules alone get ignored, hooks alone can't handle judgment calls, so the three need to work together.
...
- Strong dependencies between subtasks, frequently sharing intermediate state: subagents aren't a good fit

---

## 8. Prompt Caching: A First-Class Design Constraint

This topic is discussed less often in tutorials, but it has a major effect on Claude Code's cost structure and design choices. Claude Code's entire architecture is built around prompt caching. A high cache hit rate cuts cost, improves latency, and loosens rate limits. Anthropic monitors hit rates internally and treats low cache performance as an incident-level signal.

### Prompt Layout Designed for Caching

<img src="https://cdn.tw93.fun/blog/upic/tCDytz.png" width="1000px" alt="Lay Out Your Prompt for Caching">
...
<img src="https://cdn.tw93.fun/blog/upic/0M2eXx.png" width="1000px" alt="Forking Context: Compaction">
...
When writing task prompts or skills, define acceptance criteria upfront: which commands must pass, what to check first if they fail, and what screenshots and logs should show. The earlier you make these clear, the less trouble you'll have later.
...
<img src="https://cdn.tw93.fun/blog/upic/XwDs5n.png" width="1000px" alt="/mcp connection status showing server tool counts and token consumption">
...
<img src="https://cdn.tw93.fun/blog/upic/6IysXR.jpg" width="1000px" alt="Keep it simple">
...
One of my favorite tricks: after correcting Claude, have it update its own `CLAUDE.md`:
...
Claude Code calls real shells, git, package managers, and local configurations. As long as one layer is opaque, it has to start guessing, and once it starts guessing about the environment, reliability drops fast.
...
I also found that when the CLI exposes subcommands with clear purposes such as `init`, `config`, and `reset`, Claude Code uses them more reliably than when it has to infer where configuration files live. Get the state under control before adding ways to edit it. Reversing that order creates unnecessary chaos.

### Hooks Practice for Mixed-Language Projects

Two languages need two sets of checks, and Hooks are a good way to run each set for the relevant file type:
...
Based on the six-layer framework in this article, I packaged these checks into an open-source skill project, [`tw93/waza`](https://github.com/tw93/waza), which can evaluate your Claude Code configuration in one pass.
...
If you want to know how far your configuration is from these principles after reading this article, running `/health` is the fastest way.
...
These are a few things I've learned from six months of tinkering. There's plenty I haven't explored yet. If you've found tricks that work even better, I'd love to hear them.
