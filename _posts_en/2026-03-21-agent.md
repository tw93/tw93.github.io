...
The core of an Agent Loop comes down to fewer than 20 lines of code:
...
The corresponding control flow cycles through four stages: perceive, decide, act, and receive feedback, repeating until the model returns plain text:
...
Selection mainly comes down to two factors: task determinism and whether verification can be automated.

| Scenario | Pick |
| --- | --- |
| Fixed flow + code-verifiable | Workflow / Prompt Chaining, no agent needed |
| Input splits into distinct branches | Routing |
| Needs reasoning + clear acceptance criteria | Single Agent ReAct Loop |
| Decomposable + parallelizable subtasks + summary-only result | Orchestrator-Workers, main ReAct + sub-agents |
| Quality hard to codify (translation, creative work) | Evaluator-Optimizer |
| High-stakes decisions + multiple perspectives | Parallelization (voting) |

Main agents use ReAct Loop with an explicit task graph. Sub-agents get a minimal prompt (Tooling, Workspace, Runtime) without Skills or Memory to avoid leaking privileges or breaking isolation. Multi-agent is not the default. Find out what a single agent can handle first, since coordination overhead often outweighs the parallelism gain.
...
Transformer attention scales at $O(n^2)$. Longer context means signals get diluted by noise. The most common failure mode is Context Rot: irrelevant content crowds the window and decision quality drops. Many apparent model failures trace back to poor context organization. The Claude Code team places the onset of Context Rot around 300k-400k tokens on 1M-context models, though this varies heavily with task type.
...
Compression is reactive. The Claude Code team documented five proactive ways to manage a session:

- **continue**: send another message in the same session. Most natural, most abused.
- **rewind**: double-tap Esc or run `/rewind` to go back to a previous turn; everything after it is dropped.
- **clear**: start a new session with a brief you wrote yourself.
- **compact**: let the model summarize the session and keep going on top of the summary.
- **subagents**: delegate the next chunk of work to an agent with its own clean context; only the result comes back.

Rewind often beats correction when things go wrong. If Claude read five files, tried an approach, and hit a wall, tacking on "that didn't work, try X instead" keeps the failed reasoning in context. Rewinding to just after the file reads and re-prompting with what you learned works better. Claude Code also supports "summarize from here" to generate a handoff note before rewinding.

Compact and clear both trim weight, but differently. Compact hands the decision to the model, which is convenient but may drop things you actually care about. Clear takes more effort, and what survives is exactly what you decided to keep.
...
The left shows a tool that only explains what it does, never when to use or avoid it. The Agent picks the wrong tool, gets the parameters wrong, and keeps retrying. The right follows ACI: clear boundaries, structured errors with recovery hints, and the Agent usually gets it on the first call.
...
Keep progress in files, not in the context. Use JSON for feature lists instead of Markdown, since structured formats are much easier for the model to reliably modify. The task is only considered complete when every feature in `feature-list.json` reads `passes: true`.
...
As autonomy grows, what slows the main loop is usually external I/O: file operations, network requests, long-running shell commands. When these block the main loop, they slow the whole task down.
...
The first questions in multi-agent engineering are isolation and coordination. There are two distinct ways of working here.
...
4. **Prioritize Skills over new tools**: Managing domain knowledge via documents is more flexible than writing new executable tools.
...
Most of what looks like an LLM capability ceiling is a failure to establish engineering constraints. When an agent misbehaves, walk the diagnosis ladder from cheapest to most expensive: check tool descriptions first, then whether task state is externalized, then whether the eval system itself is distorted, and only then consider peeling deterministic parts out into a Workflow. Most problems are fixed in the first two steps; there's rarely a need to jump straight to prompt rewrites or model swaps.
...
Here's a short recap for later reference. If you have other Agent development experiences or approaches that work better, I'd love to hear them.
...
9. For observability, Traces are the foundation. Event streams should publish once to multiple consumers. Calibrate automated LLM scoring against human annotations, using both together.
