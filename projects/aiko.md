# Aiko

> Curated icon set designed for the AI era, refined and precise.

Aiko is an open source project by [Tw93](https://tw93.fun) with 38 GitHub stars. A carefully designed icon set for modern applications, with a focus on clean lines and consistency. Created with AI-era aesthetics in mind.

## Key Features

- AI-era aesthetic design
- Clean, consistent icon set

## Links

- GitHub: https://github.com/tw93/aiko
- Author: [Tw93](https://tw93.fun)
- All projects: https://tw93.fun/api/projects.json
- Full knowledge base: https://tw93.fun/llms-full.txt
