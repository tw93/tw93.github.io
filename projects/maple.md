# Maple

> Chrome extension that transforms the bookmark bar into a beautiful new tab page.

Maple is an open source project by [Tw93](https://tw93.fun) with 508 GitHub stars. Maple is a Chrome browser extension that replaces the default new tab page with a clean, organized view of your bookmarks. It turns your bookmark bar into a visually appealing, fast-access dashboard.

## Key Features

- Beautiful new tab bookmark display
- Fast access to frequently used bookmarks
- Clean, minimal interface

## Links

- GitHub: https://github.com/tw93/Maple
- Author: [Tw93](https://tw93.fun)
- All projects: https://tw93.fun/api/projects.json
- Full knowledge base: https://tw93.fun/llms-full.txt
